...
### ParaView VTU Workflow
When the goal is visualization rather than repartitioning, prefer `tri2vtk_converter.py` for any mesh project folder that already contains `file.prj`, a `.tri`, and `.par` files.

For raw Gmsh `.msh` input, use `workflow/converters/msh_to_vtk.py` instead. That path generates `.vtk` and `.tri`, not `main.vtu`.
...
- ParaView-ready `.vtu` files generated by `tri2vtk_converter.py` as `main.vtu` in the case folder
- Raw `.msh` conversions produced by `workflow/converters/msh_to_vtk.py` as `.vtk` and `.tri`
